...
public class TargetMarkerManager : MonoBehaviour
{
    ...
    {
        DestroyMarkers();
        for (int i = 0; i < Bot.orderPosList.Count; i++)
        {
            GameObject marker = Instantiate(clickedTargetPrefab, Bot.orderPosList[i] + new Vector3(0, 0.1f, 0), Quaternion.Euler(new Vector3(90,0,0)));
